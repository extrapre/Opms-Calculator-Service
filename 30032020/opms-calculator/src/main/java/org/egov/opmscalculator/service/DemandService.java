...
import static org.egov.opmscalculator.utils.OPMSCalculatorConstants.MDMS_ROUNDOFF_TAXHEAD;
...
import org.egov.opmscalculator.repository.DemandRepository;
import org.egov.opmscalculator.repository.ServiceRequestRepository;
import org.egov.opmscalculator.repository.builder.CalculationQueryBuilder;
import org.egov.opmscalculator.utils.CalculationUtils;
import org.egov.opmscalculator.utils.OPMSCalculatorConstants;
import org.egov.opmscalculator.web.models.Calculation;
import org.egov.opmscalculator.web.models.RequestInfoWrapper;
import org.egov.opmscalculator.web.models.demand.Demand;
import org.egov.opmscalculator.web.models.demand.DemandDetail;
import org.egov.opmscalculator.web.models.demand.DemandResponse;
...
public class DemandService {
	...
	private List<Demand> createDemand(RequestInfo requestInfo, List<Calculation> calculations, Object mdmsData) {
		...
		for (Calculation calculation : calculations) {
			OpmsDetail opmsDetail = null;

			if (calculation.getOpmsDetail() != null)
				opmsDetail = calculation.getOpmsDetail();
			else
			...
			User owner = opmsDetail.getOwners().get(0).toCommonUser();
			...
			Map<String, Long> taxPeriods = mdmsService.getTaxPeriods(requestInfo, opmsDetail, mdmsData);
			...
			demands.add(Demand.builder().consumerCode(consumerCode).demandDetails(demandDetails).payer(owner)
					...
					.taxPeriodFrom(taxPeriods.get(OPMSCalculatorConstants.MDMS_STARTDATE))
					.taxPeriodTo(taxPeriods.get(OPMSCalculatorConstants.MDMS_ENDDATE))
					.consumerType(opmsDetail.getApplicationType()).businessService(config.getBusinessService())
					.build());
		}
